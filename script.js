...
const apikey='e6017ae1ece368f466e3744bef784516';
    var now=new Date();
    ...
    if((hours>=18 && hours<=24)||(hours>=0 && hours<=5)){
        bg.style.backgroundImage ='url("images/night.jpeg")'
    ...
    document.addEventListener('DOMContentLoaded',()=>{
        fetch(`http://api.openweathermap.org/geo/1.0/direct?q=Faridabad,IN&appid=${apikey}`)
        .then((res)=>res.json())
        .then((data) => {
          ...
          .then((data)=>{
            ...
            if(air<=5 && air>3){
                image.src='images/air-pollution.png';
                AIQ.style.color='red';
            }
            else if(air==3){
